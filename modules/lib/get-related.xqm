...
declare function rel:build-relationships($node as item()*,$idno as xs:string?, $relationship-type as xs:string?, $display as xs:string?, $map as xs:string?, $label as xs:string?){
    ...
    let $relationships := 
                        if($relationship-type != '') then
                            if($relationship-type = 'dcterms:subject') then
                                $node/descendant-or-self::tei:relation[@ref=$relationship-type][not(@ana=('site-type','building-type'))]
                            else $node/descendant-or-self::tei:relation[@ref=$relationship-type]
          ...
          <div class="panel panel-default relationships" xmlns="http://www.w3.org/1999/xhtml">
            ...
            <div class="panel-body">{(
                    ...
                    if($relationship-type != '') then 
                        ...
                        return 
                            try{
                                if($display = 'list-description') then
                                    ...
                                    return 
                                        (<ul class="indent">{(
                                         for $r in subsequence(tokenize($names,' ')[not(. = $idno)],1,10)
                                         let $data := $related-map($r)
                                         let $title := if($data/descendant::tei:place/tei:placeName) then $data/descendant::tei:place/tei:placeName[1]/text() else $data/descendant::tei:titleStmt/tei:title[1]/text()
                                         where not(empty($data))
                                         return 
                                            <li><div class="short-rec-view feature">{
                                                    (<span class="feature-title">
                                                        <a href="{replace($r,$config:base-uri,$config:nav-base)}" dir="ltr">{$title}</a>
                                                        {if($related/descendant::tei:desc != '') then concat(' (',normalize-space(string-join($related/descendant::tei:desc/descendant-or-self::text(),'')),')') else ()}
                                                        </span>)
                                                }</div></li>,
                                        if($count gt 10) then
                                                <li><span>
                        ...
                        for $related at $n in $relationships
                        let $rel-id := index-of($node, $related[1])
                        let $rel-type := if($related/@ref) then $related/@ref else $related/@name
                        ...
                        return 
                            if($display = 'list-description') then
                                ...
                                let $count := count(tokenize($names,' ')[not(. = $idno)])
                                return 
                                    (if($relationship-type != '') then () else <span class="relationship-type">{rel:get-names($idno, $related-map)}&#160;{rel:translate-relationship-type($rel-type)} ({$count})</span>,
                                         <ul class="indent">
                                         {(
                                         for $r in subsequence(tokenize($names,' ')[not(. = $idno)],1,10)
                                         let $data := $related-map($r)
                                         ...
                                         if($count gt 10) then
                                                <span>
                                                    <span class="collapse" id="showRel-{$rel-id}">{
                                                        ...
                                                        let $data := $related-map($r)
                                                        let $title := if($data/descendant::tei:place/tei:placeName) then $data/descendant::tei:place/tei:placeName[1]/text() else $data/descendant::tei:titleStmt/tei:title[1]/text()
                                                        return <li><div class="short-rec-view feature">{
                                                            (<span class="feature-title">
                                                                <a href="{replace($r,$config:base-uri,$config:nav-base)}" dir="ltr">{$title}</a>
                                                                {if($related/descendant::tei:desc != '') then concat(' (',normalize-space(string-join($related/descendant::tei:desc/descendant-or-self::text(),'')),')') else ()}
                                                                </span>)
