<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.tei-c.org/ns/1.0" xmlns:x="http://www.w3.org/1999/xhtml" xmlns:saxon="http://saxon.sf.net/" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:local="http://syriaca.org/ns" exclude-result-prefixes="xs t x saxon local" version="2.0">
 ...
 <!-- =================================================================== -->
    ...
    <xsl:template match="t:place | t:person | t:bibl[starts-with(@xml:id,'work-')] | t:entryFree">
        ...
        <xsl:if test="t:state">
            <xsl:for-each-group select="//t:state" group-by="@type">
                ...
                <h4>
                    <xsl:choose>
                        <xsl:when test="self::t:state[@type='existence']">Dynasty <xsl:if test="self::t:state/@subtype != 'dynasty'"><xsl:value-of select="concat(upper-case(substring(self::t:state/@subtype,1,1)),substring(self::t:state/@subtype,2))"/></xsl:if>:</xsl:when>
                        <xsl:otherwise>
    ...
    <xsl:template match="t:birth | t:death | t:floruit | t:sex | t:langKnowledge">
        <span class="srp-label">
            <xsl:choose>
